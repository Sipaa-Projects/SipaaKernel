...
el_status ElfLoader::PRead(el_ctx *ctx, void *def, size_t nb, size_t offset)
{
    return ctx->pread(ctx, def, nb, offset) ? EL_OK : EL_EIO;
}

el_status ElfLoader::FindPHDR(el_ctx *ctx, Elf_Phdr *phdr, uint32_t type, unsigned *i)
{
    el_status rv = EL_OK;
    for (; *i < ctx->ehdr.e_phnum; (*i)++) {
        if ((rv = PRead(ctx, phdr, sizeof(*phdr), EL_PHOFF(ctx, *i))))
            return rv;
...
el_status ElfLoader::Init(el_ctx *ctx)
{
    el_status rv = EL_OK;
    if ((rv = PRead(ctx, &ctx->ehdr, sizeof(ctx->ehdr), 0)))
        return rv;
    ...
    unsigned i = 0;
    for(;;) {
        if ((rv = FindPHDR(ctx, &ph, PT_LOAD, &i)))
            return rv;
    ...
    if (ctx->ehdr.e_type == ET_DYN) {
        ...
        if ((rv = FindPHDR(ctx, &ph, PT_DYNAMIC, &i)))
            return rv;
...
el_status ElfLoader::Load(el_ctx *ctx, el_alloc_cb alloc)
{
    el_status rv = EL_OK;
    ...
    for(;;) {
        ...
        /* read loaded portion */
        if ((rv = PRead(ctx, dest, ph.p_filesz, ph.p_offset)))
            return rv;

        /* zero mem-only portion */
        BasicMemoryManagement::MemorySet32(dest + ph.p_filesz, 0, ph.p_memsz - ph.p_filesz);
...
el_status ElfLoader::FindDyn(el_ctx *ctx, Elf_Dyn *dyn, uint32_t tag)
{
    el_status rv = EL_OK;
    size_t ndyn =  ctx->dynsize / sizeof(Elf_Dyn);

    for(unsigned i = 0; i < ndyn; i++) {
        if ((rv = PRead(ctx, dyn, sizeof(*dyn), ctx->dynoff + i * sizeof(*dyn))))
            return rv;
...
el_status ElfLoader::FindRelocs(el_ctx *ctx, el_relocinfo *ri, uint32_t type)
{
    el_status rv = EL_OK;

    Elf_Dyn rel, relsz, relent;

    if ((rv = FindDyn(ctx, &rel, type)))
        return rv;

    if ((rv = FindDyn(ctx, &relsz, type + 1)))
        return rv;

    if ((rv = FindDyn(ctx, &relent, type + 2)))
        return rv;
...
el_status ElfLoader::Relocate(el_ctx *ctx)
{
    el_status rv = EL_OK;
    ...
    if ((rv = FindRelocs(ctx, &ri, DT_REL)))
        return rv;
    ...
    for (size_t i = 0; i < relcnt; i++) {
        if ((rv = Reloc::Apply(ctx, &reltab[i])))
            return rv;
    }
#else
    ...
    if ((rv = FindRelocs(ctx, &ri, DT_RELA)))
        return rv;
    ...
    for (size_t i = 0; i < relacnt; i++) {
        if ((rv = Reloc::ApplyA(ctx, &relatab[i])))
            return rv;
    }
#else
